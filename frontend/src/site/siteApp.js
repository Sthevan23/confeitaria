...
export async function bootSite() {
  forceStartAtTop();
  try {
...
function initLoader() {
  ...
  // Evita loader travado se algum asset falhar
...
function initSettings() {
  ...
  const name = settings.name || 'Flor de Açúcar';
  const tagline = settings.tagline || 'Encomenda ou pronta entrega — escolha o bolo no cardápio e finalize seu pedido em minutos.';
  const address = settings.address || '';

  // Nome e textos principais
  document.title = `${name} — Confeitaria Artesanal`;
  setText('logo-text', name);
  setText('loader-text', name);
  setText('footer-logo-text', name);
  setText('footer-name', name);
  setText('footer-tagline', `${tagline}${address ? '. Confeitaria artesanal no ' + address + '.' : '.'}`);

  // Hero
  const heroBg = document.getElementById('hero-bg');
  const banner = settings.banner || 'imagens/hero.jpg?v10';
  if (heroBg) {
    heroBg.style.backgroundImage = `url('${banner}')`;
  }
  setText('hero-brand', name);
  setText('hero-lead', tagline);
  ...
  setText('sobre-address', address);
  const sobreImg = document.getElementById('sobre-img');
  if (sobreImg) {
    sobreImg.src = settings.sobreImage || 'imagens/loja.jpg?v9';
    sobreImg.alt = `Bolos artesanais ${name}`;
  }
  ...
  // Contato (WhatsApp só como canal de contato, não como atalho de pedido)
  const waNumber = settings.whatsapp || '5531999999999';
  const waUrl = `https://wa.me/${waNumber}?text=${encodeURIComponent('Olá! Vi o site da ' + name + ' e gostaria de tirar uma dúvida.')}`;
  setHref('contact-whatsapp', waUrl);
  setHref('footer-whatsapp', waUrl);
  setText('contact-whatsapp', formatPhone(waNumber));
...
function initProducts() {
  ...
  function renderReadyDeliveryInfo() {
    return `
      <div class="ready-delivery-banner reveal visible" data-category="cat3">
        ...
        <a href="#produtos" class="btn btn--primary btn--sm ready-delivery-banner__cta">Ver bolos do dia</a>
      </div>
      <div class="ready-delivery-info reveal visible" data-category="cat3">
        ...
        <article>
          ...
          <h4>PIX na hora</h4>
          <p>Pagamento rápido na retirada.</p>
        </article>
      </div>
    `;
  }
      ...
      <article class="product-card reveal visible${hiddenClass}" data-category="${product.categoryId}" data-product-id="${product.id}" data-index="${index}">
        ...
        <div class="product-card__body">
          ...
          <div class="product-card__footer">
            ...
            <a href="#pedido" class="btn btn--primary btn--sm btn-pedir" data-product="${product.name}" data-price="${product.price}" data-category="${product.categoryId}"><i class="fas fa-shopping-bag"></i> Pedir</a>
          </div>
        </div>
      </article>
    `;
  ...
  // Pedir abre o configurador no site (sem atalho direto ao WhatsApp)
...
function setupExtraMenu(settings) {
  ...
  menuWhatsapp.addEventListener('click', async (e) => {
    ...
    menuWhatsapp.innerHTML = '<i class="fas fa-check"></i> Finalizar encomenda';
  });
    ...
    return `
      <div class="featured-card reveal visible">
        ...
        <div class="featured-card__body">
          ...
          <a href="#pedido" class="btn btn--primary btn--sm">
